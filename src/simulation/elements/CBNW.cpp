	...
	for (rx=-1; rx<2; rx++)
		for (ry=-1; ry<2; ry++)
			if ((rx || ry) && x+rx>=0 && y+ry>=0 && x+rx<XRES && y+ry<YRES)
			{
				...
				if (!r)
				{
					if (sim->pv[y / CELL][x / CELL] + parts[i].temp > -273.15f || sim->rng.chance(1, 4000))
					{
						sim->part_change_type(i, x, y, PT_CO2);
						parts[i].ctype = 5;
				...
				int rt = TYP(r);
				if ((sim->elements()[rt].Properties & TYPE_PART) && parts[i].tmp == 0 && sim->rng.chance(1, 83))
				{
					//Start explode
					parts[i].tmp = sim->rng.between(0, 24);
				}
				else if((sim->elements()[rt].Properties & TYPE_SOLID) && rt != PT_DMND && rt != PT_GLAS && parts[i].tmp == 0 && sim->rng.chance(int(2 - sim->pv[y/CELL][x/CELL]), 6667))
				{
					sim->part_change_type(i, x, y, PT_CO2);
					parts[i].ctype = 5;
					sim->pv[y/CELL][x/CELL] += 0.2f;
